...
group.add_argument('--model', default='vit_small_patch16_224', type=str, metavar='MODEL',
                   help='Name of model to train (default: "vit_small_patch16_224")')
...
def main():
    ...
    if "mu" in args.opt:
        if "vit_base_patch16_224" in args.model:
            ...
            model = mu_vit(hidden_dim=768,mlp_dim=3072,num_classes=args.num_classes)
            base_model = mu_vit(hidden_dim=24,mlp_dim=24*4,num_classes=args.num_classes)
            delta_model = mu_vit(hidden_dim=36,mlp_dim=36*4,num_classes=args.num_classes)
            set_base_shapes(model, base_model, delta=delta_model, override_base_dim=1)
            model.reset_parameters_all()
            model.num_classes = args.num_classes if args.num_classes is not None else 1000
        elif "vit_small_patch16_224" in args.model:
            from mu_vit import mu_vit
            from mup import set_base_shapes
            model = mu_vit(image_size=224, patch_size=16, num_layers=12, num_heads=6, hidden_dim=384, mlp_dim=1536, num_classes=args.num_classes)
            base_model = mu_vit(image_size=224, patch_size=16, num_layers=12, num_heads=6, hidden_dim=24, mlp_dim=24*4, num_classes=args.num_classes)
            delta_model = mu_vit(image_size=224, patch_size=16, num_layers=12, num_heads=6, hidden_dim=36, mlp_dim=36*4, num_classes=args.num_classes)
            set_base_shapes(model, base_model, delta=delta_model, override_base_dim=1)
            model.reset_parameters_all()
            model.num_classes = args.num_classes if args.num_classes is not None else 1000
        elif "vit_large_patch16_224" in args.model:
            from mu_vit import mu_vit
            from mup import set_base_shapes
            model = mu_vit(image_size=224, patch_size=16, num_layers=24, num_heads=16, hidden_dim=1024, mlp_dim=4096, num_classes=args.num_classes)
            base_model = mu_vit(image_size=224, patch_size=16, num_layers=24, num_heads=6, hidden_dim=24, mlp_dim=24*4, num_classes=args.num_classes)
            delta_model = mu_vit(image_size=224, patch_size=16, num_layers=24, num_heads=6, hidden_dim=36, mlp_dim=36*4, num_classes=args.num_classes)
            set_base_shapes(model, base_model, delta=delta_model, override_base_dim=1)
            model.reset_parameters_all()
            model.num_classes = args.num_classes if args.num_classes is not None else 1000
    else:
    ...
    if "lo" in args.opt:
        ...
        if args.opt == "velo":
            ...
            optimizer = MuLO_CUDA(model.parameters(), lr=args.lr,weight_decay=args.weight_decay)
    else:
        optimizer = create_optimizer_v2(
            model,
            **optimizer_kwargs(cfg=args),
    ...
    if use_amp == 'apex':
        ...
        # Initialize NVIDIA Apex AMP (Automatic Mixed Precision) with optimization level O1
        # This converts the model and optimizer to use mixed precision training
        # O1 is a balanced mode that uses FP16 for most ops while keeping some in FP32
    ...
    if utils.is_primary(args):
        ...
        if args.log_wandb:
            if has_wandb:
                ...
                # if wandb_id.txt exist in output_dir
                if os.path.exists(os.path.join(output_dir, 'wandb_id.txt')):
                    with open(os.path.join(output_dir, 'wandb_id.txt'), 'r') as f:
                        args.wandb_resume_id = f.read().strip()
                wandb.init(
    ...
    if utils.is_primary(args):
        ...
        _logger.info(
            ...
            f'LR stepped per {"epoch" if lr_scheduler is not None and hasattr(lr_scheduler,"t_in_epoch") else "update"}.')
        _logger.info(
            f"Using lr schedulers {lr_scheduler} with {updates_per_epoch} updates per epoch, "
        )
    ...
    try:
        for epoch in range(start_epoch, num_epochs):
            ...
            if TERMINATE_FLAG:
                ...
                break
            current_epoch_max_steps = (epoch + 1) * updates_per_epoch
...
def train_one_epoch(
    ...
    for batch_idx, (input, target) in enumerate(loader):
        ...
        if TERMINATE_FLAG:
            if utils.is_primary(args):
                print("Signal received, terminating at next epoch")
                
        current_step = epoch * updates_per_epoch + update_idx
